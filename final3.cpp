...
for(int i = 0; i < members.size(); i++) {
    for(int j = 0; j < members.size(); j++) {
        ...
        balances[i] -= expenses[j][i]; // add the expenses that are owed to member i by others
    }
}
    ...
    // Display the debts owed by each debtor
    for(int i = 0; i < debtors.size(); i++) {
    ...
    // Display a message for members who do not owe or are owed money
    for(int i = 0; i < balances.size(); i++) {
        if(balances[i] == 0) {
            cout << members[i] << " does not owe or is owed money." << endl;
        }
    }
}
